...
@interface UnionFileSelector(){
    
}
@property (nonatomic, copy) NSString* fullDir;
@end

@implementation UnionFileSelector
...
- (BOOL) reload{
    ...
    // 判断文件夹是否存在，如果不存在，则创建
    if (![fmgr fileExistsAtPath:_fullDir]) {
        [fmgr createDirectoryAtPath:_fullDir
    ...
    return [self selectFileWithType:UnionSelectType_NEXT];
}

/**
 @abstract   获取一个音频文件
*/
- (BOOL)selectFileWithType:(UnionSelectType)type{
    ...
    if (type == UnionSelectType_NEXT) {
        //next
        _fileIdx = (_fileIdx+1)%cnt;
    }
    else if (type == UnionSelectType_RANDOM){
        //random
        _fileIdx = (NSInteger)(arc4random() % cnt);
    }
    else if (type == UnionSelectType_PREVIOUS){
        //previous
