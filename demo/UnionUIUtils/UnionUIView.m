...
#pragma mark - init
- (id) initWithParent:(UnionUIView*)pView{
    self = [self init];
    if (pView){
        self.hidden = YES;
...
- (UnionNameSlider *)addSliderName: (NSString*) name
                            From: (float) minV
                              To: (float) maxV
                            Init: (float) iniV {
    UnionNameSlider *sl = [[UnionNameSlider alloc] init];
    [self addSubview:sl];
...
- (IBAction)onBtn:(id)sender {
    ...
    if ([self.superview isKindOfClass:[UnionUIView class]]){
        UnionUIView * v = (UnionUIView *)self.superview;
        [v onBtn:sender];
    }
}
